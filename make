...
class Simple_Ajax_Search_Generator
{
    public $dir =__DIR__;
        ...
        */
        ...
        while (!empty($dirStack)) {
            ...
            while (false !== ($node = $dir->read())) {
                if ( ($node == 'includes') ||  ($node == 'admin') || ($node == 'LICENSE.txt') || ($node == '.vscode') || ($node == 'tests') || ($node == 'node_modules') || ($node == 'vendor') || ($node == 'tests') ||($node == 'phpunit.xml') ||($node == 'gruntfile.js') ||($node == 'gulpfile.js') ||($node == 'package.json') || ($node == '..') || ($node == '.') || ($node=='.git')  || ($node=='composer.json') || ($node=='composer.lock') || ($node=='.gitignore') || ($node =='simple-ajax-search-generator') || ($node=='readme.md') || ($node=='bitbucket-pipelines.yml') || ($node=='.gitmodules') || ($node=='.editorconfig') || ($node=='.jshintrc') || ($node=='make') ) {
                    continue;
                }
 ...
 * @subpackage Simple_Ajax_Search/includes
 */
 ...
 */
class Simple_Ajax_Search_$data[2] {
    ...
    public static function name_of_the_function() {
        ...
        file_put_contents(__DIR__."/$data[3]/class-simple-ajax-search-$data[2].php", $input);
        exit("File class-simple-ajax-search-$data[2].php created into $data[3] folder");
    }

}

$plugin_name_generator = new Simple_Ajax_Search_Generator();
